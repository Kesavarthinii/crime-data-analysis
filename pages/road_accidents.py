...
# ---------------- PAGE CONFIG & THEME ----------------
st.set_page_config(page_title="Road Accidents Analysis", layout="wide")
...
st.markdown(
    "<h1 style='text-align:center; font-size:2.6rem; margin-bottom:8px; color:white;'>ROAD ACCIDENT CASES (2020 - 2022)</h1>",
    unsafe_allow_html=True
)

# ---------------- SIDEBAR ----------------
st.sidebar.markdown("<h3 style='color:#FFD700;'>Data & GeoJSON (optional)</h3>", unsafe_allow_html=True)
uploaded_file = st.sidebar.file_uploader("Upload Road Accidents Excel (.xlsx)", type=["xlsx"])
...
def load_excel(path=None):
    if path is not None:
        ...
        df = pd.read_excel(r"C:\Users\kesav\OneDrive\DWM\crime-data-mining\data\Road Accidents.xlsx")
...
with col2:
    ...
    selected_viz = st.selectbox("Select Visualization (order fixed)", viz_options)
...
# KEEPING THE EXACT SAME RANGES FROM ORIGINAL ROAD ACCIDENTS CODE
def intensity_color(v):
    if v > 20000:
        return "#FF2400"  # Red
    elif 15000 < v <= 20000:
        return "#FF7F0E"  # Dark Orange
    elif 10000 < v <= 15000:
        return "#FFD700"  # Yellow
    elif 5000 < v <= 10000:
        return "#FFCC80"  # Light Orange
    elif 1000 < v <= 5000:
        return "#FFE0B2"  # Soft Peach
    else:
        return "#FFF2E5"  # Very Light Peach (<1000)
...
def attach_counts(geojson, data_df):
    for feat in geojson.get('features', []):
        ...
        props['Count'] = val
        props['hover'] = f"{st_nm.title() if st_nm else 'Unknown'}: {val:,} accidents"
...
if selected_viz == "Choropleth Map":
    ...
    for feat in geo.get('features', []):
        ...
        if coords:
            ...
            colors.append(props.get('color', '#FFF2E5'))
    ...
    fig.update_layout(
        ...
        title=dict(text=f"🗺 Road Accident Cases Across India — {selected_year}", x=0.5, font=dict(size=18, color='white'))
    )
    ...
    with col_leg:
        ...
        st.markdown("""
            <div style="background-color:#FFF2E5; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ Below 1000 (Very Low)</div>
            <div style="background-color:#FFE0B2; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 1000 – 5000 (Low)</div>
            <div style="background-color:#FFCC80; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 5000 – 10000 (Moderate)</div>
            <div style="background-color:#FFD700; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 10000 – 15000 (High)</div>
            <div style="background-color:#FF7F0E; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 15000 – 20000 (Very High)</div>
            <div style="background-color:#FF2400; padding:6px; margin:5px; border-radius:6px; color:white;">⬜ Above 20000 (Severe)</div>
        """, unsafe_allow_html=True)
...
# ---------------- 2) TOP 10 STATES — TABLE ----------------
    ...
    st.markdown("### 🔝 Top 10 States by Total Accident Cases (2020–2022)", unsafe_allow_html=True)
    ...
    fig = px.bar(top10.sort_values('Total'), x='Total', y='State/UT', orientation='h', text='Total',
                 color='Total', color_continuous_scale=['#FFF2E5','#FFE0B2','#FFCC80','#FFD700','#FF7F0E','#FF2400'])
    ...
    fig.update_layout(
        ...
        xaxis_title="Total Accident Cases (2020-2022)",
...
# ---------------- 3) BOTTOM 5 STATES — TABLE ----------------
    ...
    st.markdown("### 🔻 Bottom 5 States by Total Accident Cases (2020–2022)", unsafe_allow_html=True)
...
# ---------------- 4) HORIZONTAL BAR CHART (SELECTED YEAR) ----------------
    ...
    fig = px.bar(sorted_df, x='Count', y='State/UT', orientation='h', text='Count',
                 color='Count', color_continuous_scale=['#FFF2E5','#FFE0B2','#FFCC80','#FFD700','#FF7F0E','#FF2400'])
    fig.update_traces(textposition='outside')
    fig.update_layout(
        paper_bgcolor="#121212", 
        plot_bgcolor="#121212", 
        ...
        height=800, 
        margin=dict(l=50,r=20,t=30,b=20),
        xaxis_title=f"Number of Accident Cases ({selected_year})",
        yaxis_title="State/UT"
    )
    st.plotly_chart(fig, use_container_width=True)
...
# ---------------- 5) GROUPED BAR CHART (2020–2022) ----------------
    ...
    fig.update_layout(
        ...
        yaxis_title="Number of Accident Cases"
    )
...
with st.expander("📌 Key Insights (2020 - 2022)"):
    ...
    # Additional insights from your original code
    avg_accidents = df[['2020','2021','2022']].mean().mean()
    df['growth'] = df['2022'] - df['2020']
    fastest_increase = df.sort_values('growth', ascending=False).iloc[0]['State/UT']
    sharpest_drop = df.sort_values('growth', ascending=True).iloc[0]['State/UT']

    st.markdown(f"""
    <div style="background:#1e1e1e;padding:16px;border-radius:8px;color:#FFD700;">
        <p><b>Total accident cases (2020–2022):</b> {total:,}</p>
        ...
        <p><b>Average per year:</b> {avg_accidents:.2f}</p>
        <p><b>Fastest increase (2020 → 2022):</b> {fastest_increase}</p>
        <p><b>Sharpest decline:</b> {sharpest_drop}</p>
        <p><b>Overall Trend (2020 → 2022):</b> {trend} ({abs(inc_2022):,} cases)</p>
    </div>
    """, unsafe_allow_html=True)