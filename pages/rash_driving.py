...
# ---------------- PAGE CONFIG & THEME ----------------
st.set_page_config(page_title="Rash Driving Analysis", layout="wide")
...
st.markdown(
    "<h1 style='text-align:center; font-size:2.6rem; margin-bottom:8px; color:white;'>RASH DRIVING CASES (2020 - 2022)</h1>",
    unsafe_allow_html=True
)

# ---------------- SIDEBAR ----------------
st.sidebar.markdown("<h3 style='color:#FFD700;'>Data & GeoJSON (optional)</h3>", unsafe_allow_html=True)
uploaded_file = st.sidebar.file_uploader("Upload Rash Driving Excel (.xlsx)", type=["xlsx"])
...
def load_excel(path=None):
    if path is not None:
        ...
        df = pd.read_excel(r"C:\Users\kesav\OneDrive\DWM\crime-data-mining\data\Rash Driving.xlsx")
...
# KEEPING THE EXACT SAME RANGES FROM ORIGINAL RASH DRIVING CODE
...
if selected_viz == "Choropleth Map":
    ...
    fig.update_layout(
        ...
        title=dict(text=f"🗺 Rash Driving Cases Across India — {selected_year}", x=0.5, font=dict(size=18, color='white'))
    )
    ...
    with col_leg:
        ...
        st.markdown("""
            <div style="background-color:#FFEFD5; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ Below 10000 (Very Low)</div>
            <div style="background-color:#FFCC80; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 10000 – 25000 (Low)</div>
            <div style="background-color:#FFD700; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 25000 – 50000 (Moderate)</div>
            <div style="background-color:#FF7F0E; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 50000 – 70000 (High)</div>
            <div style="background-color:#FF2400; padding:6px; margin:5px; border-radius:6px; color:white;">⬜ Above 70000 (Very High)</div>
        """, unsafe_allow_html=True)
...
# ---------------- 2) TOP 10 STATES — TABLE ----------------
    ...
    fig.update_layout(
        ...
        xaxis_title="Total Cases (2020-2022)",
...
# ---------------- 4) HORIZONTAL BAR CHART (SELECTED YEAR) ----------------
    ...
    fig.update_layout(
        ...
        height=800, 
        margin=dict(l=50,r=20,t=30,b=20),
        xaxis_title=f"Number of Cases ({selected_year})",
        yaxis_title="State/UT"
    )
    st.plotly_chart(fig, use_container_width=True)
...
with st.expander("📌 Key Insights (2020 - 2022)"):
    ...
    <div style="background:#1e1e1e;padding:16px;border-radius:8px;color:#FFD700;">
        <p><b>Total rash driving cases (2020–2022):</b> {total:,}</p>
