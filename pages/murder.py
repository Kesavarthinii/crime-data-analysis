...
# ---------------- PAGE CONFIG & THEME ----------------
st.set_page_config(page_title="Murder Analysis", layout="wide")
...
st.markdown(
    "<h1 style='text-align:center; font-size:2.6rem; margin-bottom:8px; color:white;'>MURDER CASES (2020 - 2022)</h1>",
    unsafe_allow_html=True
)

# ---------------- SIDEBAR ----------------
st.sidebar.markdown("<h3 style='color:#FFD700;'>Data & GeoJSON (optional)</h3>", unsafe_allow_html=True)
uploaded_file = st.sidebar.file_uploader("Upload Murder Excel (.xlsx)", type=["xlsx"])
...
def load_excel(path=None):
    if path is not None:
        ...
        df = pd.read_excel(r"C:\Users\kesav\OneDrive\DWM\crime-data-mining\data\Murder.xlsx")
...
# KEEPING THE EXACT SAME RANGES FROM ORIGINAL MURDER CODE
...
def attach_counts(geojson, data_df):
    for feat in geojson.get('features', []):
        ...
        props['Count'] = val
        props['hover'] = f"{st_nm.title() if st_nm else 'Unknown'}: {val:,} murders"
...
if selected_viz == "Choropleth Map":
    ...
    fig.update_layout(
        ...
        title=dict(text=f"🗺 Murder Cases Across India — {selected_year}", x=0.5, font=dict(size=18, color='white'))
    )
    ...
    with col_leg:
        ...
        st.markdown("""
            <div style="background-color:#FFEFD5; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ Below 500 (Very Low)</div>
            <div style="background-color:#FFCC80; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 500 – 1000 (Low)</div>
            <div style="background-color:#FFD700; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 1000 – 2000 (Moderate)</div>
            <div style="background-color:#FF7F0E; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 2000 – 3000 (High)</div>
            <div style="background-color:#FF2400; padding:6px; margin:5px; border-radius:6px; color:white;">⬜ Above 3000 (Very High)</div>
        """, unsafe_allow_html=True)
...
# ---------------- 2) TOP 10 STATES — TABLE ----------------
    ...
    st.markdown("### 🔝 Top 10 States by Total Murder Cases (2020–2022)", unsafe_allow_html=True)
    ...
    fig = px.bar(top10.sort_values('Total'), x='Total', y='State/UT', orientation='h', text='Total',
                 color='Total', color_continuous_scale=['#FFEFD5','#FFCC80','#FFD700','#FFAA00','#FF7F0E','#FF2400'])
    ...
    fig.update_layout(
        ...
        xaxis_title="Total Murder Cases (2020-2022)",
...
# ---------------- 3) BOTTOM 5 STATES — TABLE ----------------
    ...
    st.markdown("### 🔻 Bottom 5 States by Total Murder Cases (2020–2022)", unsafe_allow_html=True)
...
# ---------------- 4) HORIZONTAL BAR CHART (SELECTED YEAR) ----------------
    ...
    fig = px.bar(sorted_df, x='Count', y='State/UT', orientation='h', text='Count',
                 color='Count', color_continuous_scale=['#FFEFD5','#FFCC80','#FFD700','#FFAA00','#FF7F0E','#FF2400'])
    fig.update_traces(textposition='outside')
    fig.update_layout(
        paper_bgcolor="#121212", 
        plot_bgcolor="#121212", 
        ...
        height=800, 
        margin=dict(l=50,r=20,t=30,b=20),
        xaxis_title=f"Number of Murder Cases ({selected_year})",
        yaxis_title="State/UT"
    )
    st.plotly_chart(fig, use_container_width=True)
...
# ---------------- 5) GROUPED BAR CHART (2020–2022) ----------------
    ...
    fig.update_layout(
        ...
        yaxis_title="Number of Murder Cases"
    )
...
with st.expander("📌 Key Insights (2020 - 2022)"):
    ...
    <div style="background:#1e1e1e;padding:16px;border-radius:8px;color:#FFD700;">
        <p><b>Total murder cases (2020–2022):</b> {total:,}</p>
