...
# ---------------- PAGE CONFIG & THEME ----------------
st.set_page_config(page_title="Missing Persons Analysis", layout="wide")
...
st.markdown(
    "<h1 style='text-align:center; font-size:2.6rem; margin-bottom:8px; color:white;'>MISSING PERSON CASES (2020 - 2022)</h1>",
    unsafe_allow_html=True
)

# ---------------- SIDEBAR ----------------
st.sidebar.markdown("<h3 style='color:#FFD700;'>Data & GeoJSON (optional)</h3>", unsafe_allow_html=True)
uploaded_file = st.sidebar.file_uploader("Upload Missing Persons Excel (.xlsx)", type=["xlsx"])
...
def load_excel(path=None):
    if path is not None:
        ...
        df = pd.read_excel(r"C:\Users\kesav\OneDrive\DWM\crime-data-mining\data\Missing Persons.xlsx")
...
if selected_viz == "Choropleth Map":
    ...
    fig.update_layout(
        ...
        title=dict(text=f"🗺 Missing Persons Across India — {selected_year}", x=0.5, font=dict(size=18, color='white'))
    )
...
# ---------------- 2) TOP 10 STATES — TABLE ----------------
    ...
    fig.update_layout(paper_bgcolor="#121212", plot_bgcolor="#121212", font_color='white', height=700, margin=dict(l=50,r=20,t=30,b=20))
...
# ---------------- 3) BOTTOM 5 STATES — TABLE ----------------
    ...
    st.dataframe(bottom5[['State/UT','2020','2021','2022']].rename(columns={'State/UT':'State/UT'}), height=300)
...
# ---------------- 4) HORIZONTAL BAR CHART (SELECTED YEAR) ----------------
    ...
    fig.update_traces(textposition='outside')
    fig.update_layout(paper_bgcolor="#121212", plot_bgcolor="#121212", font_color='white', height=860, margin=dict(l=50,r=20,t=30,b=20))
    st.plotly_chart(fig, use_container_width=True)
...
# ---------------- 5) GROUPED BAR CHART (2020–2022) ----------------
    ...
    fig.update_layout(paper_bgcolor="#121212", plot_bgcolor="#121212", font_color='white', height=880, margin=dict(l=60,r=20,t=50,b=90))
...
# ---------------- 6) SLOPE GRAPH — STATE-WISE TREND ----------------
    ...
    fig.update_layout(paper_bgcolor="#121212", plot_bgcolor="#121212", font_color='white', height=600, margin=dict(t=40,b=20))
    st.plotly_chart(fig, use_container_width=True)
...
with st.expander("📌 Key Insights (2020 - 2022)"):
    ...
    <div style="background:#1e1e1e;padding:16px;border-radius:8px;color:#FFD700;">
        <p><b>Total missing persons (2020–2022):</b> {total:,}</p>
