...
# ---------------- PAGE CONFIG & THEME ----------------
st.set_page_config(page_title="Sexual Harassment Analysis", layout="wide")
...
st.markdown(
    "<h1 style='text-align:center; font-size:2.6rem; margin-bottom:8px; color:white;'>SEXUAL HARASSMENT CASES (2020 - 2022)</h1>",
    unsafe_allow_html=True
)

# ---------------- SIDEBAR ----------------
st.sidebar.markdown("<h3 style='color:#FFD700;'>Data & GeoJSON (optional)</h3>", unsafe_allow_html=True)
uploaded_file = st.sidebar.file_uploader("Upload Sexual Harassment Excel (.xlsx)", type=["xlsx"])
...
def load_excel(path=None):
    if path is not None:
        ...
        df = pd.read_excel(r"C:\Users\kesav\OneDrive\DWM\crime-data-mining\data\Sexual Harassment.xlsx")
...
# KEEPING THE EXACT SAME RANGES FROM ORIGINAL SEXUAL HARASSMENT CODE
def intensity_color(v):
    if v > 20000:
        return "#FF2400"  # Dark Red
    elif 15000 < v <= 20000:
        ...
        return "#FFE0B2"  # Light Peach
    else:
        return "#FFF5E6"  # Very light (<1000)
...
if selected_viz == "Choropleth Map":
    ...
    fig.update_layout(
        ...
        title=dict(text=f"🗺 Sexual Harassment Cases Across India — {selected_year}", x=0.5, font=dict(size=18, color='white'))
    )
    ...
    with col_leg:
        ...
        st.markdown("""
            <div style="background-color:#FFF5E6; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ Below 1000</div>
            <div style="background-color:#FFE0B2; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 1000 – 5000</div>
            <div style="background-color:#FFCC80; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 5000 – 10000</div>
            <div style="background-color:#FFD700; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 10000 – 15000</div>
            <div style="background-color:#FF7F0E; padding:6px; margin:5px; border-radius:6px; color:black;">⬜ 15000 – 20000</div>
            <div style="background-color:#FF2400; padding:6px; margin:5px; border-radius:6px; color:white;">⬜ Above 20000</div>
        """, unsafe_allow_html=True)
...
# ---------------- 2) TOP 10 STATES — TABLE ----------------
    ...
    fig = px.bar(top10.sort_values('Total'), x='Total', y='State/UT', orientation='h', text='Total',
                 color='Total', color_continuous_scale=['#FFF5E6','#FFE0B2','#FFCC80','#FFD700','#FF7F0E','#FF2400'])
    ...
    fig.update_layout(
        ...
        xaxis_title="Total Sexual Harassment Cases (2020-2022)",
...
# ---------------- 3) BOTTOM 5 STATES — TABLE ----------------
    ...
    st.markdown("### 🔻 Bottom 5 States by Total Cases (2020–2022)", unsafe_allow_html=True)
...
# ---------------- 4) HORIZONTAL BAR CHART (SELECTED YEAR) ----------------
    ...
    fig = px.bar(sorted_df, x='Count', y='State/UT', orientation='h', text='Count',
                 color='Count', color_continuous_scale=['#FFF5E6','#FFE0B2','#FFCC80','#FFD700','#FF7F0E','#FF2400'])
    fig.update_traces(textposition='outside')
    fig.update_layout(
        paper_bgcolor="#121212", 
        plot_bgcolor="#121212", 
        ...
        height=800, 
        margin=dict(l=50,r=20,t=30,b=20),
        xaxis_title=f"Number of Sexual Harassment Cases ({selected_year})",
        yaxis_title="State/UT"
    )
    st.plotly_chart(fig, use_container_width=True)
...
# ---------------- 5) GROUPED BAR CHART (2020–2022) ----------------
    ...
    fig.update_layout(
        ...
        yaxis_title="Number of Sexual Harassment Cases"
    )
...
with st.expander("📌 Key Insights (2020 - 2022)"):
    ...
    <div style="background:#1e1e1e;padding:16px;border-radius:8px;color:#FFD700;">
        <p><b>Total sexual harassment cases (2020–2022):</b> {total:,}</p>
        ...
        <p><b>Trend (2020 → 2022):</b> {trend} ({abs(inc_2022):,} cases)</p>
    </div>
    """, unsafe_allow_html=True)